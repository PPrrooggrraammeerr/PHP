    ...
    if (isset($_POST['editar'])) {  
        ...
        if (isset($_FILES['imagem'])) {
            ...
            if (move_uploaded_file($imagem_temporaria, $caminho_imagem)) {
                ...
                $sql = 'SELECT imagem FROM cardapio_cafe WHERE id = :id';
        ...
        if ($tipo === 'Café') {
            ...
            if ($cardapio_cafe->editar_cafe($id, $nome, $tipo, $descricao, $preco, $caminho_imagem)) {
        ...
        if ($id > 0) {
            ...
            $sql = 'SELECT * FROM cardapio_cafe WHERE id = :id';
            $stmt = $pdo->prepare($sql);
...
<body>
    <main>
        ...
        <section class="container-form">
            <form action="editar_produto_cafe.php" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="id" value="<?php echo ($exteriorizar['id']);?>">
                <label for="nome">Nome</label>
                <input type="text" id="nome" name="nome" placeholder="Digite o nome do produto" value="<?php echo ($exteriorizar['cafe']);?>" required>
